...
export function UserProvider({ children}) {
 ...
 // Sync selectedCareerPath state after login/logout
 // After login, localStorage may have been cleared so re-read it;
 // if it's gone, fall back to the user's saved career id so it's never blank.
 useEffect(() => {
 if (user?.id) {
 // User just logged in — re-read from storage (written during prior session)
 const stored = localStorage.getItem('selectedCareerPath') || sessionStorage.getItem('selectedCareerPath') || '';
 setSelectedCareerPathState(stored);
} else {
 // User logged out — clear in-memory state
 setSelectedCareerPathState('');
}
}, [user?.id]);
