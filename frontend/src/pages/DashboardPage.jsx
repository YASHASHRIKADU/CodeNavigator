...
export default function DashboardPage() {
    ...
    const statCards = [
        ...
        { label: 'In Progress', value: stats.inProgress, icon: Clock, colorClass: 'bg-amber-100 dark:bg-amber-900/20', iconColor: 'text-amber-500' },
        { label: 'Total Skills', value: stats.total, icon: BookOpen, colorClass: 'bg-blue-50 dark:bg-blue-900/20', iconColor: 'text-blue-500' },
        { label: 'Day Streak', value: '5 🔥', icon: Flame, colorClass: 'bg-orange-50 dark:bg-orange-900/20', iconColor: 'text-orange-500' },
    ];
        ...
        <DashboardLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold font-poppins text-text-primary dark:text-gray-100">
                ...
                {!user?.career ? (
                    ...
                    <div className="card text-center py-16">
                        ...
                        <h2 className="text-xl font-bold font-poppins text-text-primary dark:text-gray-100 mb-2">Let's get you started</h2>
                        <p className="text-text-secondary dark:text-gray-400 mb-6 max-w-sm mx-auto">Choose your career goal and we'll generate a personalized learning roadmap for you.</p>
                    ...
                    <>
                        ...
                        <div className="grid lg:grid-cols-3 gap-6">
                            ...
                            <div className="card flex flex-col items-center py-8 lg:col-span-1">
                                <h2 className="font-bold text-text-primary dark:text-gray-100 font-poppins mb-6">Overall Progress</h2>
                                <ProgressRing value={stats.completed} max={stats.total || 1} size={140} strokeWidth={12} />
                                <p className="text-text-secondary dark:text-gray-400 text-sm mt-4 text-center">
                            ...
                            <div className="lg:col-span-2 space-y-4">
                                ...
                                {/* Recent activity */}
                                <div className="card">
                                    <h3 className="font-bold text-text-primary dark:text-gray-100 font-poppins mb-4">Recent Activity</h3>
                                    {recentActivity.length === 0 ? (
                                        <p className="text-text-secondary dark:text-gray-400 text-sm text-center py-4">
                                            No activity yet. Start your first skill!
                                        </p>
                                    ) : (
                                        <div className="space-y-2">
                                            {recentActivity.slice(0, 5).map(({ skill, status }) => (
                                                <div key={skill.skillId} className="flex items-center gap-3 p-2 rounded-xl hover:bg-bg-light dark:hover:bg-primary-900/20 transition-colors">
                                                    <div className={clsx(
                                                        'w-2 h-2 rounded-full flex-shrink-0',
                                                        status === 'completed' ? 'bg-success' : 'bg-amber-400'
                                                    )} />
                                                    <span className="text-sm text-text-primary dark:text-gray-100 flex-1">{skill.name}</span>
                                                    <span className={clsx(
                        ...
                        {roadmap && (
                            <div className="card">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="font-bold text-text-primary dark:text-gray-100 font-poppins">Roadmap Stages</h3>
                                ...
                                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                    {roadmap.stages.map((stage, idx) => {
                                        ...
                                        return (
                                            <Link to="/roadmap" key={stage.title} className="p-4 rounded-xl bg-bg-light dark:bg-primary-900/20 hover:bg-primary-50 dark:hover:bg-primary-900/30 transition-colors group">
                                                ...
                                                <p className="text-xs font-bold text-text-primary dark:text-gray-100 mb-2">{stage.title}</p>
                                                <div className="h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                                                    <div className="h-full bg-primary-500 rounded-full transition-all" style={{ width: `${pct}%` }} />
                                                </div>
                                                <p className="text-xs text-text-secondary dark:text-gray-400 mt-1">{done}/{stage.skills.length} done</p>
                                            </Link>
                                        );
                                    })}
                                </div>
