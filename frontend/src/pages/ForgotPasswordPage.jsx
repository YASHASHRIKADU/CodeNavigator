            ...
            {Array.from({ length: 6 }).map((_, idx) => (
                <input
                    ...
                    className={`w-11 h-12 text-center text-lg font-bold rounded-xl border-2 transition-all duration-200 outline-none
                        ${digits[idx]
                            ? 'border-primary-500 bg-primary-500/5 text-text-primary dark:text-white'
                            : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-text-primary dark:text-white'}
            ...
            <div className="p-2">
                    ...
                    {['Email', 'Verify OTP', 'Reset'].map((label, i) => (
                        <div key={label} className="flex items-center gap-1.5">
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all
                                ${step > i + 1 ? 'bg-green-500 text-white' : step === i + 1 ? 'bg-primary-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-400'}`}>
                                {step > i + 1 ? '✓' : i + 1}
                            </div>
                            <span className={`text-xs font-medium ${step === i + 1 ? 'text-primary-500' : 'text-gray-400'}`}>{label}</span>
                            {i < 2 && <div className={`flex-1 h-px w-6 ${step > i + 1 ? 'bg-green-400' : 'bg-gray-200 dark:bg-gray-700'}`} />}
                        </div>
                    ))}
                </div>
                ...
                {step === 2 && (
                    <div className="space-y-5">
                        <p className="text-text-secondary dark:text-gray-400 text-sm">
                            We sent a 6-digit OTP to <strong className="text-text-primary dark:text-white">{email}</strong>.
                            Enter it below.
                        </p>
                        ...
                        <button type="button" onClick={() => { setStep(1); setOtp(''); setError(''); }}
                            className="w-full text-sm text-text-secondary dark:text-gray-400 hover:text-primary-500 transition-colors">
                            ← Change email
                        </button>
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="text-center space-y-3 py-4">
                        ...
                        <p className="font-semibold text-text-primary dark:text-white">OTP verified!</p>
                        <p className="text-sm text-text-secondary dark:text-gray-400">Redirecting to password reset...</p>
