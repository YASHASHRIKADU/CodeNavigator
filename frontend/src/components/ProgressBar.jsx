        ...
        <div className={clsx('w-full', className)}>
            {(label || showPercent) && (
                <div className="flex justify-between items-center mb-1.5">
                    {label && <span className="text-sm font-medium text-text-primary dark:text-gray-200">{label}</span>}
                    {showPercent && (
                        <span className="text-sm font-bold text-primary-500">{pct}%</span>
                    )}
                </div>
            )}
            <div className={clsx('w-full bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden', heightClass)}>
                <div
    ...
    return (
        <div className="relative inline-flex items-center justify-center">
            ...
            <div className="absolute text-center">
                <div className="text-xl font-black text-text-primary dark:text-gray-100 font-poppins">{pct}%</div>
                {sublabel && <div className="text-xs text-text-secondary dark:text-gray-400">{sublabel}</div>}
            </div>
        </div>
    );
}
